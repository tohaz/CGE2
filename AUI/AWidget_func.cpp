  ...
  void DrawTextEx(uint32_t *buffer, uint32_t parentWidth, uint32_t parentHeight,
      ...
      for (char c : text) {
          ...
          if (c >= 32 && c <= 126) {
              ...
              if (pre) { asc = pre->asc; desc = pre->desc; found = true; }
          }
          if (!found) {
              ...
              if (it != metricsCache.end()) { asc = it->second.first; desc = it->second.second; }
              else {
                  ...
                  } else { asc = static_cast<int32_t>(fontSize); desc = 0; }
              }
          }
      ...
      if (textHeight == 0) textHeight = static_cast<int32_t>(fontSize);
      int32_t startX = absX - xOffset;
      if (hAlign == AUIHAlign::center) startX += (maxContentWidth - totalWidth) / 2;
      else if (hAlign == AUIHAlign::right) startX += (maxContentWidth - totalWidth);
      int32_t baselineY = absY;
      if (vAlign == AUIVAlign::center) baselineY += (drawH - textHeight) / 2;
      else if (vAlign == AUIVAlign::bottom) baselineY += drawH - textHeight;
      ...
      for (char c : text) {
          if (c >= 32 && c <= 126) {
              ...
              if (pre) {
                  ...
                  if (glyphRight <= clipL) { penX += pre->advance; continue; }
                  if (glyphLeft >= clipR) break;
                  ...
                  if (glyphBottom > clipT && glyphTop < clipB) {
                      ...
                      for (size_t row = 0; row < rows; ++row) {
                          ...
                          if (destY < clipT || destY >= clipB) continue;
                          ...
                          for (size_t col = 0; col < cols; ++col) {
                              ...
                              if (destX < clipL || destX >= clipR) continue;
...
//      return;
//    FT_Set_Pixel_Sizes(face, 0, fontSize);
//// ----- Measure ascent/descent and total width -----
//    int32_t maxAscent = 0, maxDescent = 0, totalWidth = 0;
//    for(char c : text) {
//      FT_UInt glyph_index = FT_Get_Char_Index(face, static_cast<FT_ULong>(c));
//      FT_Fixed advance = engine->GetCachedAdvance(glyph_index, fontSize);
...
//        }
//      }
//// Fallback to metrics cache or load on the fly
//      if(!found) {
//        uint64_t mkey = (static_cast<uint64_t>(glyph_index) << 32) | fontSize;
//        auto &metricsCache = engine->GetMetricsCache();
//        auto it = metricsCache.find(mkey);
...
//        }
//        else {
//// Load without rendering just to get metrics
//          if(FT_Load_Char(face, static_cast<FT_ULong>(c), FT_LOAD_NO_HINTING) == 0) {
//            FT_GlyphSlot slot = face->glyph;
//            asc = static_cast<int32_t>(slot->bitmap_top);
//            desc = static_cast<int32_t>(slot->bitmap.rows) - asc;
...
//    baselineY += textHeight;// baseline is at bottom of text
//// ----- Clipping rectangle -----
//    int32_t clipL = absX;
//    int32_t clipR = absX + drawW;
//    int32_t clipT = absY;
//    int32_t clipB = absY + drawH;
//    const size_t pW = static_cast<size_t>(parentWidth);
//    const size_t totalPixels = pW * static_cast<size_t>(parentHeight);
//    uint32_t colorNoAlpha = textColor & 0x00FFFFFFU;
//    const uint32_t col_r = (colorNoAlpha >> 16) & 0xFF;
...
//    int32_t penX = startX;
//    for(char c : text) {
//// ----- Pre‑rendered ASCII path (fast, safe) -----
//      if(c >= 32 && c <= 126) {
//        const CachedGlyph *pre = engine->GetPreRenderedGlyph(static_cast<uint8_t>(c), fontSize);
//        if(pre) {
//          int32_t glyphLeft = penX + pre->left;
...
//        }
//      }
//// ----- Fallback: load and render directly (no cache, no manual free) -----
//      if(FT_Load_Char(face, static_cast<FT_ULong>(c), FT_LOAD_RENDER | FT_LOAD_NO_HINTING) != 0)
//        continue;
//      FT_GlyphSlot slot = face->glyph;
//      FT_Bitmap *bitmap = &slot->bitmap;
//// If the glyph is not a bitmap (should not happen with FT_LOAD_RENDER), skip.
//      if(bitmap->pixel_mode != FT_PIXEL_MODE_GRAY)
//        continue;
//      int32_t glyphLeft = penX + slot->bitmap_left;
//      int32_t glyphRight = glyphLeft + static_cast<int32_t>(bitmap->width);
//      if(glyphRight <= clipL) {
...
//      if(glyphLeft >= clipR)
//        break;
//      int32_t glyphTop = baselineY - slot->bitmap_top;
//      int32_t glyphBottom = glyphTop + static_cast<int32_t>(bitmap->rows);
//      if(glyphBottom > clipT && glyphTop < clipB) {
//        const size_t rows = static_cast<size_t>(bitmap->rows);
//        const size_t cols = static_cast<size_t>(bitmap->width);
...
//        }
//      }
//// Advance pen position using the slot's advance
//      penX += static_cast<int32_t>(slot->advance.x >> 6);
//    }
//  }
