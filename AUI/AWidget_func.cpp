...
// Bresenham's line algorithm
    ...
    int32_t err = dx + dy;
    while (true) {
// Clip pixel to buffer bounds
      if(x0 >= 0 && x0 < static_cast<int32_t>(bufferWidth) && y0 >= 0 && y0 < static_cast<int32_t>(bufferHeight)) {
        buffer[static_cast<size_t>(y0) * bufferWidth + static_cast<size_t>(x0)] = color;
      }
      if(x0 == x1 && y0 == y1)
        break;
      int32_t e2 = 2 * err;
