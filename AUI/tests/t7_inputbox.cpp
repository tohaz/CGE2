...
// ------------------------------------------------------------------
int32_t test_input_basic_ops() {
    D1("test_input_basic_ops start");
    AUI *au = AUI::Create("InputBasicTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    ...
    // Insert character
    input->SetText("Hello");
    input->SetCursorPos(2);   // between 'l' and 'l'
    send_char(input, 'x');
    TEST_ASSERT_EQ(input->GetText(), "Hexllo", 2);
    TEST_ASSERT_EQ(input->GetCursorPos(), 3U, 3);   // cursor after 'x'
...
// ------------------------------------------------------------------
int32_t test_input_max_length() {
    D1("test_input_max_length start");
    AUI *au = AUI::Create("InputMaxLenTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    ...
    input->SetText("abc");
    input->SetCursorPos(3);   // move cursor to end
    send_char(input, 'd');
    send_char(input, 'e');
...
// Input filter (regex)
// ------------------------------------------------------------------
int32_t test_input_filter() {
    D1("test_input_filter start");
    AUI *au = AUI::Create("InputFilterTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    // Allow only digits
    ...
    input->SetCursorPos(4);   // after the '4'
    send_char(input, 'b');
...
// Cursor movement
// ------------------------------------------------------------------
int32_t test_input_cursor() {
    D1("test_input_cursor start");
    AUI *au = AUI::Create("InputCursorTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    input->SetText("ABCDE");
...
// ------------------------------------------------------------------
int32_t test_insert_mode() {
    D1("test_insert_mode start");
    AUI *au = AUI::Create("InputInsertModeTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    // Insert mode (default)
...
// ------------------------------------------------------------------
int32_t test_editable() {
    D1("test_editable start");
    AUI *au = AUI::Create("InputEditableTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    input->SetText("Hello");
...
// ------------------------------------------------------------------
int32_t test_enabled() {
    D1("test_enabled start");
    AUI *au = AUI::Create("InputEnabledTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    ...
    input->SetCursorPos(1);
    send_char(input, 'X');
//    TEST_ASSERT_EQ(input->GetText(), "EEnabled", 4);
...
// ------------------------------------------------------------------
int32_t test_callbacks() {
    D1("test_callbacks start");
    AUI *au = AUI::Create("InputCallbackTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    ...
    input->SetText("abc");
    input->SetCursorPos(3);  // <-- ADD THIS (move cursor to end)
...
// ------------------------------------------------------------------
int32_t test_alignment() {
    D1("test_alignment start");
    AUI *au = AUI::Create("InputAlignTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    input->SetText("Hi");
    ...
    // For an empty text, cursor should be at right edge.
    // We cannot directly check pixel X here, but we can check GetCursorX non‑crash.
...
int32_t test_mouse_click() {
    D1("test_mouse_click start");
    AUI *au = AUI::Create("InputClickTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    input->SetText("ABCD");
    ...
    // The actual X coordinate depends on font metrics; we just test that cursor moves.
    ...
    // No crash is the main success criterion
    delete au;
...
// ------------------------------------------------------------------
int32_t test_backspace_edge_cases() {
    D1("test_backspace_edge_cases start");
    AUI *au = AUI::Create("InputBackspaceTest");
    TEST_ASSERT(au != nullptr, 1);
    AWindow *w = au->MainWnd();
    AInputBox *input = AInputBox::AttachTo(w);
    ...
    input->SetText("xyz");
    input->SetCursorPos(3);  // after last character
    send_key(input, AUIKeyCode::Backspace);  // deletes 'z'
    TEST_ASSERT_EQ(input->GetText(), "xy", 6);
    send_key(input, AUIKeyCode::Backspace);  // deletes 'y'
    TEST_ASSERT_EQ(input->GetText(), "x", 7);
    send_key(input, AUIKeyCode::Backspace);  // deletes 'x'
    TEST_ASSERT_EQ(input->GetText(), std::string(""), 8);
    TEST_ASSERT_EQ(input->GetCursorPos(), 0U, 9);
    
    // Case 4: Backspace in overwrite mode (should behave same as insert mode)
    input->SetText("12345");
    input->SetInsertMode(false);  // overwrite mode
    input->SetCursorPos(3);       // before the 4th character
    send_key(input, AUIKeyCode::Backspace);
    TEST_ASSERT_EQ(input->GetText(), "1245", 10);  // deletes '3'
...
int32_t test_insert_overwrite_modes() {
    AUI *au = AUI::Create("InsertOverwriteTest");
    AInputBox *input = AInputBox::AttachTo(au->MainWnd());
    input->SetText("12345");
    input->SetCursorPos(2);
    // Insert mode (default)
    send_char(input, 'X');
...
int main() {
    ...
    AUI *au = AUI::Create("aui input box test");
    ...
    testsfailed += test_input_attachment();
    testsfailed += test_input_basic_ops();
    testsfailed += test_input_max_length();
    testsfailed += test_input_filter();
    testsfailed += test_input_cursor();
    testsfailed += test_insert_mode();
    testsfailed += test_editable();
    testsfailed += test_enabled();
    testsfailed += test_callbacks();
    testsfailed += test_alignment();
    testsfailed += test_mouse_click();
    testsfailed += test_backspace_edge_cases();
    testsfailed += test_insert_overwrite_modes();
