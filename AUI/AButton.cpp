...
namespace aui {
  ...
  void AButton::OnDraw(uint32_t* buffer, uint32_t bufferW, uint32_t bufferH,
                       int32_t offsetX, int32_t offsetY,
                       int32_t clipL, int32_t clipT, int32_t clipR, int32_t clipB) const {
      D2("bufferW %u bufferH %u offsetX %d offsetY %d clip=(%d,%d)-(%d,%d)",
      ...
      if (!Text().empty() && face) {
          ...
          if (btnClipL < btnClipR && btnClipT < btnClipB) {
              ARect clipBounds{ btnClipL, btnClipT,
                                static_cast<uint32_t>(btnClipR - btnClipL),
