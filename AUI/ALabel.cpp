...
namespace aui {
  ...
  void ALabel::Draw(uint32_t *buffer, uint32_t parentWidth, uint32_t parentHeight, int32_t offsetX, int32_t offsetY) const {
    ...
    int32_t maxX = std::min(static_cast<int32_t>(parentWidth) - 1, static_cast<int32_t>(std::max({cornerX1, cornerX2, cornerX3, cornerX4})));
    int32_t minY = std::max(0, static_cast<int32_t>(std::min({cornerY1, cornerY2, cornerY3, cornerY4})));
    int32_t maxY = std::min(static_cast<int32_t>(parentHeight) - 1, static_cast<int32_t>(std::max({cornerY1, cornerY2, cornerY3, cornerY4})));
    for (int32_t screenY = minY; screenY <= maxY; ++screenY) {
      ...
      for (int32_t screenX = minX; screenX <= maxX; ++screenX) {
        ...
        if (localX >= 0.0 && localX < static_cast<double>(mSizeX) && localY >= 0.0 && localY < static_cast<double>(mSizeY)) {
